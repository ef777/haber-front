...
export async function getTemaAyarlari() {
  try {
    const response = await api.get('/tema-ayarlari', {
      params: {
        'populate': '*',
      },
    });
    ...
    console.error('getTemaAyarlari error:', error);
    return null;
  }
}
...
export async function getYorumlar(haberId) {
  try {
    const response = await api.get(`/yorumlar/haber/${haberId}`);
...
export async function postYorum(haberId, data) {
  try {
    const response = await api.post(`/yorumlar/haber/${haberId}`, data);
    return response.data;
  } catch (error) {
    console.error('postYorum error:', error);
    throw error;
  }
}

export async function likeYorum(yorumId) {
  try {
    const response = await api.post(`/yorumlar/${yorumId}/like`);
    return response.data;
  } catch (error) {
    console.error('likeYorum error:', error);
    throw error;
  }
}
